...
> `--descriptors`, `--format`, and `--native-api` flags, the `--client` flag on
> `go` and `python` ([streaming-extensions.md](../streaming-extensions.md)), and
> the `add-rpc` and `debug-wit-dir` subcommands — is
